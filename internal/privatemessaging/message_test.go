...
func TestSendConfirmMessageE2EOk(t *testing.T) {
	...
	mdi.On("UpsertMessage", pm.ctx, mock.Anything, database.UpsertOptimizationNew, true).Return(nil).Once()
...
func TestSendMessageFail(t *testing.T) {
	...
	mdi.On("UpsertMessage", pm.ctx, mock.Anything, database.UpsertOptimizationNew, true).Return(fmt.Errorf("pop"))
...
func TestSendUnpinnedMessageInsertFail(t *testing.T) {
	...
	mdi := pm.database.(*databasemocks.Plugin)
	mdi.On("UpsertMessage", pm.ctx, mock.Anything, database.UpsertOptimizationNew, true).Return(fmt.Errorf("pop")).Once()
