...
func newAddressResolver(ctx context.Context, localConfig config.Section, cacheManager cache.Manager, enableCache bool) (ar *addressResolver, err error) {
	...
	// The ethereum address resolver can only access a top level field in the response, so when we
	// make things common this enhancement should be included
	responseFieldString := localConfig.GetString(AddressResolverResponseField)

	ar = &addressResolver{
		method:        localConfig.GetString(AddressResolverMethod),
		responseField: strings.Split(responseFieldString, "."),
		client:        client,
	}
	if enableCache {
...
func (ar *addressResolver) ResolveSigningKey(ctx context.Context, keyDescriptor string, intent blockchain.ResolveKeyIntent) (string, error) {
	...
	responseString := ""
	nestedRes := jsonRes
	for _, field := range ar.responseField[:len(ar.responseField)-1] {
		nestedRes = nestedRes.GetObject(field)
	}

	responseString = nestedRes.GetString(ar.responseField[len(ar.responseField)-1])

	address, err := formatEthAddress(ctx, responseString)
	if err != nil {
