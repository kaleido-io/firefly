...
func utAddressResolverConfig() config.Section {
	coreconfig.Reset()
	config := config.RootSection("utaddressresovler")
	(&Paladin{}).InitConfig(config)
...
func TestCacheInitFail(t *testing.T) {
	...
	config := utAddressResolverConfig()
...
func newAddressResolverTestEth(t *testing.T, config config.Section) (context.Context, *Paladin, context.CancelFunc) {
	ctx, cancel := context.WithCancel(context.Background())
	cmi := &cachemocks.Manager{}
	cmi.On("GetCache", mock.Anything).Return(cache.NewUmanagedCache(ctx, 100, 5*time.Minute), nil)
	p := &Paladin{ctx: ctx}
	var err error
	p.addressResolver, err = newAddressResolver(ctx, config, cmi, true)
	assert.NoError(t, err)
	return ctx, p, cancel
}

func TestAddressResolverInPaladinOKCached(t *testing.T) {
	count := 0
	...
	config.Set(AddressResolverResponseField, "address")
	config.Set(AddressResolverMethod, "GET")
...
func TestAddressResolverNestedResponseField(t *testing.T) {
	addr := "0x256e288EDF9392B9236F698a64365F216A4Eff97"
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		rw.Header().Set("Content-Type", "application/json")
		rw.WriteHeader(200)
		rw.Write([]byte(fmt.Sprintf(`{"a": {"b": {"c": "%s"}}}`, addr)))
	}))
	defer server.Close()
	...
	config.Set(AddressResolverMethod, "GET")
	config.Set(AddressResolverURLTemplate, fmt.Sprintf("%s/resolve", server.URL))
	config.Set(AddressResolverResponseField, "a.b.c")
...
func TestAddressResolverNestedResponseFieldNotFound(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		rw.Header().Set("Content-Type", "application/json")
		rw.WriteHeader(200)
		rw.Write([]byte(`{"a": {}}`))
	...
	_, err := e.addressResolver.ResolveSigningKey(ctx, "testkeystring", blockchain.ResolveKeyIntentLookup)
	require.Error(t, err)
}
